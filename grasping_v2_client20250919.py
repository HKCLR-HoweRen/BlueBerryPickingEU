...
class TcpClient:
    def __init__(self, server_ip, server_port):
        ...
        print(f"客户端已创建，目标服务器: {server_ip}:{server_port}")
    ...
    def send_data(self, message):
        ...
        try:
            # 发送数据
            self.sockfd.sendall(message.encode('utf-8'))
    ...
    def receive_data(self):
        ...
        try:
            ...
            if data:
                print(f"从服务器接收: {data.decode('utf-8')}")
    ...
    def close_connection(self):
        # 关闭套接字
        if hasattr(self, 'sockfd') and self.sockfd:
            self.sockfd.close()
            self.connected = False
            print("连接已关闭")
...
def ScoketClient():
    ...
    # client = TcpClient("192.168.123.166", 12000)
    
    if not client.connect_to_server():
        print("无法连接到服务器，SocketClient线程退出")
        return
        
    try:
        # 发送就绪消息
        client.send_data("Arm ready")
        ...
        while True:
            ...
            if data is None:
                print("no data,waiting ...//连接断开，SocketClient线程退出")
                break
            ...
            # 发送任务完成消息
            client.send_data("no task,waiting...")
                    
    except Exception as e:
        print(f"SocketClient发生异常: {e}")
    finally:
        client.close_connection()
        print("SocketClient线程退出")
...
if __name__ == "__main__":  
    ...
    t_sck_client = threading.Thread(target=ScoketClient, args=())
    t_sck_client.start()
    task_count = 0
    ...
    try:
        while True:
            ...
            if key & 0xFF == ord('q') or key == 27: # Press esc or 'q' to close the image window
                ...
                # Arm1 go back to the sensing pose
                #robot1.moveJ(q_init, v=20, block=False)
